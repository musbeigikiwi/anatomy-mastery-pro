  ...
  try{
    ...
    if(ipinfoToken&&ip!=="unknown"){
      try{
        ...
        if(intel.ok){const d=await intel.json();countryCode=clean(d.country,8);city=clean(d.city,100);region=clean(d.region,100);const p=d.privacy||null;if(p){privacyKnown=true;vpn=!!p.vpn;tor=!!p.tor;proxy=!!p.proxy;hosting=!!p.hosting}}
      }catch{}
    }
    ...
    let score=0;if(body.newDevice)score+=20;if(body.newCountry)score+=20;if(vpn)score+=20;if(proxy)score+=25;if(hosting)score+=20;if(tor)score+=60;if(!secureContext)score+=50;if(legacyBrowser)score+=20;if(webdriver)score+=10;score=Math.min(score,100);
    ...
    if(sessionId)await admin.from("session_activity").update({ip_hash:ipHash,country_code:countryCode,city,region,vpn,tor,user_agent_summary:ua}).eq("id",sessionId).eq("user_id",user.id);
