...
(async()=>{
  ...
  try{
    ...
    if(profileError||!profile||profile.status!=="active"){
      ...
      location.replace(fallback+"?status=pending");return
    }
    window.AMPRO_AUTH={client,session,profile};
    const access=document.querySelector(".account-access");
    if(access){
      access.href=profile.role==="admin"?"admin.html":"#";
      access.innerHTML='<span class="account-shield">✓</span><span><b>'+escapeHtml(profile.full_name||session.user.email)+'</b><small>'+escapeHtml(profile.role)+' account</small></span>';
      if(profile.role!=="admin")access.addEventListener("click",e=>e.preventDefault());
    }
    const logout=document.createElement("button");
    logout.className="secure-logout";
    logout.type="button";
    logout.textContent="Sign out";
    logout.addEventListener("click",async()=>{logout.disabled=true;await client.auth.signOut();localStorage.removeItem("ampro_complete_v2");location.replace(fallback)});
