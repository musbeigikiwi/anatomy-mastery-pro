...
async function ensureTracking(){
 ...
 const {data:sid,error}=await c.rpc("start_my_session",{p_device_hash:deviceHash,p_device_label:device,p_browser_label:browser,p_os_label:os,p_user_agent:ua});
 if(!error&&sid){tracker=sid;sessionStorage.setItem("ampro_admin_session",sid)}
}
...
async function load(){
 ...
 const refreshBtn=$("refresh");if(refreshBtn){refreshBtn.disabled=true;refreshBtn.textContent="Refreshing…"}
 try{
  ...
  await ensureTracking();beginHeartbeat();beginRefreshClock();
  $("blocked").classList.remove("show");$("console").hidden=false;
  const since24=new Date(Date.now()-86400000).toISOString(),today=localDayStartISO(),activeSince=new Date(Date.now()-120000).toISOString();
  const [aq,pp,ev,ss,todayRows,totalRes,visitsRes,activeRows]=await Promise.all([
    ...
    c.from("security_events").select("id,event_type,risk_score,decision,country_code,user_agent_summary,metadata,created_at").gte("created_at",since24).order("created_at",{ascending:false}).limit(100),
    c.from("session_activity").select("id,user_id,started_at,last_seen_at,ended_at,country_code,city,region,device_label,browser_label,os_label,vpn,tor,ip_hash,profiles(full_name,role)").order("started_at",{ascending:false}).limit(100),
    ...
    c.from("session_activity").select("user_id").is("ended_at",null).gte("last_seen_at",activeSince).limit(5000)
  ]);
  const byUser=new Map();(aq.data||[]).forEach(a=>byUser.set(a.user_id,{user_id:a.user_id,created_at:a.created_at,full_name:a.profiles?.full_name||"New member"}));(pp.data||[]).forEach(x=>{if(!byUser.has(x.id))byUser.set(x.id,{user_id:x.id,created_at:x.created_at,full_name:x.full_name||"New member"})});
  const approvals=[...byUser.values()].sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)),events=ev.data||[],sessions=ss.data||[];
  const onlineUsers=new Set((activeRows.data||[]).map(x=>x.user_id)).size;
  const uniqueToday=new Set((todayRows.data||[]).map(x=>x.user_id)).size;
  $("pendingCount").textContent=approvals.length;$("activeCount").textContent=onlineUsers;$("visitsToday").textContent=visitsRes.count??0;$("uniqueToday").textContent=uniqueToday;$("totalSessions").textContent=totalRes.count??0;$("eventCount").textContent=events.length;$("riskCount").textContent=events.filter(e=>e.risk_score>=45).length;
 ...
 }finally{loading=false;if(refreshBtn){refreshBtn.disabled=false;refreshBtn.textContent="Refresh now"}}
}
...
$("refresh").onclick=()=>{secondsToRefresh=30;load()};
$("logout").onclick=async()=>{if(heartbeat)clearInterval(heartbeat);if(refreshClock)clearInterval(refreshClock);if(c&&tracker)await c.rpc("end_my_session",{p_session_id:tracker});sessionStorage.removeItem("ampro_admin_session");if(c)await c.auth.signOut();location.href="auth.html"};
window.addEventListener("pagehide",()=>{if(c&&tracker)c.rpc("touch_my_session",{p_session_id:tracker})});
load();
