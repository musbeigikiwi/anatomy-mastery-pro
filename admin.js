...
let tracker=sessionStorage.getItem("ampro_admin_session")||null,heartbeat=null;
...
async function load(){
 ...
 await ensureTracking();beginHeartbeat();
...
$("logout").onclick=async()=>{if(heartbeat)clearInterval(heartbeat);if(c&&tracker)await c.rpc("end_my_session",{p_session_id:tracker});sessionStorage.removeItem("ampro_admin_session");if(c)await c.auth.signOut();location.href="auth.html"};
